...
def Nest_Net(img_rows, img_cols, color_type=1, num_class=1, deep_supervision=True):
    ...
    conv1_2 = standard_unit(img_input_1, stage='12', nb_filter=128)
    pool1_2 = MaxPooling2D((2, 2), strides=(2, 2), name='pool1_2', data_format='channels_first')(conv1_2)

    conv2_2 = standard_unit(pool1_2, stage='22', nb_filter=256)
    pool2_2 = MaxPooling2D((2, 2), strides=(2, 2), name='pool2_2', data_format='channels_first')(conv2_2)

    # conv3_2 = res2net_bottleneck_block(pool2_2, nb_filter[1], nb_filter[2],s=int(nb_filter[1]/16.0)
    conv3_2 = standard_unit(pool2_2, stage='32', nb_filter=512)

    up8_2 = Conv2DTranspose(256, (2, 2), strides=(2, 2), name='up82', padding='same',
                            data_format='channels_first')(conv3_2)
    conv8_2 = concatenate([up8_2, conv2_2], name='merge82', axis=bn_axis)
    conv8_2 = standard_unit(conv8_2, stage='82', nb_filter=256)
    # conv8_2 = res2net_bottleneck_block(conv8_2, nb_filter[2], nb_filter[1],s=int(nb_filter[2]/16.0))

    up9_2 = Conv2DTranspose(128, (2, 2), strides=(2, 2), name='up92', padding='same',
                            data_format='channels_first')(conv8_2)
    conv9_2 = concatenate([up9_2, conv1_2], name='merge92', axis=bn_axis)
    conv9_2 = standard_unit(conv9_2, stage='92', nb_filter=128)
    # conv9_2 = res2net_bottleneck_block(conv9_2, nb_filter[1], nb_filter[0],s=int(nb_filter[1]/16.0))
    ...
    # model = multi_gpu_model(model, gpus=4)  # 就插入到这里
...
open('./UNET/architecture.json', 'w').write(json_string)
...
model_save = keras.callbacks.ModelCheckpoint(filepath='./UNET/my_model.h5', verbose=0)
best_weight = ModelCheckpoint(filepath='./UNET/best_weight.h5', verbose=1, monitor='val_loss',mode='auto', save_best_only=True)
...
model.save('./UNET/last_weight.h5', overwrite=True)
