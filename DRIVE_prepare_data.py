...
def increasing_width_height_imgs(imgs):
    assert(imgs.shape[1]==584 and imgs.shape[2]==565)
    if new_height>584:
        height_img=np.zeros((Nimgs,new_height-584,imgs.shape[2]))
        imgs=concatenate((imgs, height_img), axis=1)  # 横向连接

        width_img = np.zeros((imgs.shape[0], imgs.shape[1] ,new_width-565))
        imgs = concatenate((imgs, width_img), axis=2)  # 纵向连接
    else:
        imgs = imgs[:, int((height - new_height) / 2.0):int(height - (height - new_height) / 2.0),
               int((width - new_width) / 2.0):int(width - (width - new_width) / 2.0)]
...
def data_extend(patch_h,patch_w,per_img_patch_count,original_imgs,original_masks,original_weights):
    ...
    # 添加高斯噪声
    # extended_imgs=skimage.util.random_noise(extended_imgs, mode='gaussian', clip=True)
...
train_imgs_dir="./DRIVE/training/images"
train_groundTruth_dir="./DRIVE/training/1st_manual"
train_borderMasks_dir="./DRIVE/training/mask"
train_imgs,train_groundTruth,train_border_masks=get_datasets(train_imgs_dir,train_groundTruth_dir,train_borderMasks_dir,train_test="train")
test_imgs_dir="./DRIVE/test/images"
test_groundTruth_dir="./DRIVE/test/1st_manual"
test_borderMasks_dir="./DRIVE/test/mask"
test_imgs,test_groundTruth,test_border_masks=get_datasets(test_imgs_dir,test_groundTruth_dir,test_borderMasks_dir,train_test="test")
...
train_groundTruth=np.reshape(train_groundTruth,(Nimgs,584,565))
...
test_groundTruth=np.reshape(test_groundTruth,(Nimgs,584,565))
...
divided_train_imgs,divided_train_groundTruth=divide_test_imgs(train_imgs,train_groundTruth)
...
write_hdf5(Extended_train_imgs, "./DRIVE/u_net_data/DRIVE_dataset_imgs_train.hdf5")
write_hdf5(Extended_train_groundTruth, "./DRIVE/u_net_data/DRIVE_dataset_groundTruth_train.hdf5")

print("saving test datasets")
test_imgs=test_imgs.reshape(2000,1,patch_h,patch_w)
test_groundTruth=test_groundTruth.reshape(2000,1,patch_h,patch_w)
assert(test_imgs.shape==(2000,1,patch_h,patch_w))
print(np.min(test_imgs),np.max(test_imgs))
assert(test_groundTruth.shape==(2000,1,patch_h,patch_w) and np.min(test_groundTruth)==0 and np.max(test_groundTruth)==1)
write_hdf5(test_imgs,"./DRIVE/u_net_data/DRIVE_dataset_imgs_test.hdf5")
write_hdf5(test_groundTruth, "./DRIVE/u_net_data/DRIVE_dataset_groundTruth_test.hdf5")
